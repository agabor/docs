...
try
{
    user.SignUpAsync().ContinueWith(t => {
        if (t.IsFaulted) {
            // Errors from MSG Cloud and network interactions
...
1.  Due to limitations with Unity's `WWW` class, error details from MSG Cloud interactions are not passed back to the SDK. The error message in these scenarios is of the form ''40x message'' for example, ''400 Bad Request'' or ''404 Not Found''. You can implement a generic error handler for those scenarios.
...
In the above code, we try to fetch an object with a non-existent `ObjectId`. The MSG Cloud will return an error -- so here's how to handle it properly:
